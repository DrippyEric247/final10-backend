...
const OAUTH_ERROR_COPY = {
  ...
  apple_not_configured: 'Apple sign-in isn’t available right now.',
...
/**
 * Premium Google + Apple sign-in buttons. They are plain anchors to the backend
 * OAuth start routes (full-page redirect → provider → callback → back to app),
 * so they work without any client-side OAuth SDK.
 */
export default function SocialAuthButtons({ mode = 'login' }) {
  ...
  if (!providers.google && !providers.apple) {
    // No providers configured — render nothing but still surface OAuth errors.
    return oauthError ? (
      <div
