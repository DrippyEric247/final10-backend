...
export type SellerTrustEvidence = {
  ...
  evidence: string[];
  /** Final10 seller-specific notes — always explainable. */
...
function buildDefaultFinal10Note(state: SellerEvidenceState, sellerConcerns: string[]): string {
...
export function buildSellerTrustEvidence(
  ...
  const final10Note = buildDefaultFinal10Note(
    evidenceState,
...
export function sellerTrustEvidenceSummary(evidence: SellerTrustEvidence): string {
  const line = formatPositiveLine(evidence.positiveFeedbackPercent, evidence.feedbackCount);
  if (line) return line;
