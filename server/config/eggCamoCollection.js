/**
 * Egg Camo Collection — server mirror of @savvy/core config.
 * SOURCE OF TRUTH: packages/savvy-core/src/config/eggCamoCollection.js
 */

const EGG_CAMO_COLLECTION_VERSION = 1;

const EGG_CAMO_HATCHABLE_TIERS = Object.freeze([
  'common',
  'rare',
  'epic',
  'legendary',
...
const EGG_CAMO_IDS = Object.freeze([
  'woodland',
  'tiger',
  'arctic',
  'gold',
...
const EGG_CAMO_REQUIREMENTS = Object.freeze({
...
const EGG_CAMO_VISUALS = Object.freeze({
  woodland: Object.freeze({ rarity: 'common', accentColor: '#6b8f3f', accentColorAlt: '#3f5622' }),
  tiger: Object.freeze({ rarity: 'uncommon', accentColor: '#ff7a00', accentColorAlt: '#1a1a1a' }),
  arctic: Object.freeze({ rarity: 'rare', accentColor: '#38bdf8', accentColorAlt: '#0c4a6e' }),
  gold: Object.freeze({ rarity: 'epic', accentColor: '#fbbf24', accentColorAlt: '#92400e' }),
  diamond: Object.freeze({ rarity: 'legendary', accentColor: '#e2e8f0', accentColorAlt: '#64748b' }),
  darkNebula: Object.freeze({ rarity: 'mythic', accentColor: '#a855f7', accentColorAlt: '#4c1d95' }),
});

const EGG_CAMO_COLLECTION_MASTERED_EVENT = 'eggCamoCollectionMastered';

const EGG_CAMO_COUNTABLE_SOURCES = Object.freeze([
  'perk_machine',
  'perk_machine_hatch',
  'battle_pass',
  'supply_drop',
...
const EGG_CAMO_CATALOG_ITEMS = Object.freeze(buildEggCamoCatalogItems());
...
function evaluateEggCamoUnlock(camoId, lifetimeCollected, unlockedCamos = {}) {
...
function buildEggCamoRows(progress = {}) {
...
function summarizeEggCamoCollection(rows) {
...
function getClosestEggCamo(rows) {
  const candidates = rows
    .filter((r) => !r.unlocked)
    .map((r) => ({ ...r, remaining: Math.max(0, r.target - r.current) }))
...
function emptyEggCamoLifetimeCollected() {
  return { common: 0, rare: 0, epic: 0, legendary: 0, mythic: 0 };
}

function emptyEggCamoUnlockMap() {
  return EGG_CAMO_IDS.reduce((acc, id) => {
    acc[id] = false;
    return acc;
  }, {});
}
...
module.exports = {
  ...
  EGG_CAMO_HATCHABLE_TIERS,
  EGG_CAMO_IDS,
  EGG_CAMO_REQUIREMENTS,
  EGG_CAMO_VISUALS,
  EGG_CAMO_COLLECTION_MASTERED_EVENT,
  EGG_CAMO_COUNTABLE_SOURCES,
  EGG_CAMO_CATALOG_ITEMS,
  isHatchableEggTier,
  isEggCamoId,
