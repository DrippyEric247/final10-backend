...
function isBusinessStore(value) {
...
function isEstablishedSellerProfile(input) {
...
function isMegaReputationSeller(input) {
...
function evaluateSellerTrust(input) {
  const reasons = [];
  ...
  const feedbackCount = toNum(input.sellerFeedbackCount) ?? toNum(input.sellerFeedbackScore);
  const itemsSold = toNum(input.sellerCompletedSalesCount) ?? feedbackCount ?? null;
  ...
  const storeBiz = isBusinessStore(input.sellerAccountType);
  const returnFriendly =
    input.sellerReturnsAccepted === true ||
    String(input.sellerReturnsAccepted || '').toLowerCase() === 'true';
  ...
  if (itemsSold != null && itemsSold > 50_000) score += 10;
  else if (itemsSold != null && itemsSold > 5000) score += 7;
  else if (itemsSold != null && itemsSold > 500) score += 4;
  ...
  if (established) score = Math.max(score, 72);
  else if (mega) score = Math.max(score, 68);
